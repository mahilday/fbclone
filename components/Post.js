...
const Post = ({ name, id, message, email, image, postImage, timestamp }) => {
  const { deletePost, postSet, OpenModal, status } = useContext(MainContext);
  return (
    <div className="flex flex-col">
      <div className="p-5 mt-5 bg-white rounded-t-2xl shadow-sm">
        <div className="flex justify-between items-center space-x-2">
          ...
          <div
            onClick={() => {
              postSet(id, postImage);
              OpenModal();
            }}
            className="bg-gray-200 rounded-full p-2 hover:bg-gray-100 cursor-pointer"
          >
